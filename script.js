...
function canvas1() {
  ...
  const frameCount = 25; // Fixed frame count to match available images
  const images = [];
  const imageSeq = {
    frame: 0,
  };
  ...
  // Wait until all images are loaded before animation
  Promise.all(
    images.map(
      (img) =>
        new Promise((resolve) => {
          img.onload = resolve;
          img.onerror = resolve; // handle potential loading error
        })
    )
  ).then(() => {
    gsap.to(imageSeq, {
